...
char32_t get_code_point(const ast::Directive& d, Context& context)
{
    if (!d.get_arguments().empty()) {
        ...
        context.try_warning(
            diagnostic::U_args_ignored, pos, u8"Arguments to this directive are ignored."
        );
    }
    ...
    if (digits.empty()) {
        context.try_error(
            diagnostic::U_blank, d.get_source_span(),
    ...
    if (!value) {
        context.try_error(
            diagnostic::U_digits, d.get_source_span(),
    ...
    if (!is_scalar_value(code_point)) {
        context.try_error(
            diagnostic::U_nonscalar, d.get_source_span(),
