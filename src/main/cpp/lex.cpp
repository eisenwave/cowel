...
private:
    ...
    bool expect_markup_element(Content_Context context, std::size_t& brace_level)
    {
        ...
        const auto token_kind //
            = context == Content_Context::block  ? Token_Kind::block_text
            : context == Content_Context::string ? Token_Kind::quoted_string_text
                                                 : Token_Kind::document_text;
        emit(token_kind, m_pos.begin - initial_pos);
        return true;
    }
    ...
    void consume_quoted_string()
    {
        ...
        emit(Token_Kind::string_quote, 1);
        consume_markup_sequence(Content_Context::string);
    ...
    void consume_block()
    {
        ...
        emit(Token_Kind::brace_left, 1);

        consume_markup_sequence(Content_Context::block);

        if (expect(u8'}')) {
            emit(Token_Kind::brace_right, 1);
        }
        else {
