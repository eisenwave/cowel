...
function readModuleFileSync(file) {
...
function mapFixturePlaceholders(value, fixtureDir) {
...
describe("Document Generation", async () => {
    ...
    for (const testPath of testPaths) {
        ...
        test(relativePath, async () => {
            ...
            const mainFile = { path: testPath, data: fs.readFileSync(testPath) };
            const loadFile = (unresolvedPath, baseFileId) => {
                const resolvedPath = (() => {
                    if (baseFileId < 0) {
    ...
    for (const testPath of testPaths) {
        ...
        test(relativePath, () => {
            ...
            try {
                ...
                const result = spawnSync(process.execPath, [cliPath, "run", relativePath, outputPath, "--no-color"], {
                    cwd: cliTestDir,
                    encoding: "utf8",
                });
                assert.notStrictEqual(result.status, null, `CLI process timed out or failed to start for ${relativePath}`);
...
describe("LSP Integration (WASM)", () => {
    ...
    for (const suiteDir of suiteDirs) {
        ...
        test(suiteName, () => {
            ...
            const inputFixture = JSON.parse(fs.readFileSync(inputPath, "utf8"));
            const outputFixture = JSON.parse(fs.readFileSync(outputPath, "utf8"));
            ...
            const inputMessages = mapFixturePlaceholders(inputFixture, suiteDir);
            const expectedMessages = mapFixturePlaceholders(outputFixture, suiteDir);
            const stdinBytes = Buffer.concat(inputMessages.map((entry) => {
                if (entry !== null
                    && typeof entry === "object"
                    && "$raw" in entry
                    && typeof entry.$raw === "string") {
                    return frameJsonRpcBody(entry.$raw);
                }
            ...
            assert.strictEqual(result.status, 0, `LSP runner exited with ${result.status} for ${suiteName}: ${result.stderr.toString("utf8")}`);
